...
//	private UserApply userApply;
	...
	public String loadCollectNews(){
		...
		int fansCount = currUser.getFans().size();
		int followUserCount = currUser.getFollowUser().size();
		ActionContext.getContext().getSession().put("fansCount",fansCount);
		ActionContext.getContext().getSession().put("followUserCount", followUserCount);
	...
	public String loadMyNews(){
		User currUser = userBiz.getUser(user.getId());
		int fansCount =currUser.getFans().size();
		int followCount = currUser.getFollowUser().size();
		Set<News>myNews = currUser.getNews();
		ActionContext.getContext().getSession().put("myNews",myNews);
		ActionContext.getContext().getSession().put("fansCount",fansCount);
		ActionContext.getContext().getSession().put("followCount",followCount);
	...
	public String getDongTai(){
		User currUser = userBiz.getUser(user.getId());
		Set<News>fabuNewsSet = currUser.getNews();
		ActionContext.getContext().getSession().put("fabuNewsSet", fabuNewsSet);
		return "getDongtaiSuccess";
	}
