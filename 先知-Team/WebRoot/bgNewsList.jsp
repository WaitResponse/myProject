					...
					<c:otherwise>
						...
						<c:set var="index" value="${index+1}"/>
							<tr <c:if test="${index%2!=0 }">bgcolor="lemonchiffon"</c:if>>
								...
								<c:if test="${news.audit eq '已审核'}">
									<td ><font size="9pt" color="green">${news.audit }</font></td>
								</c:if>
								<c:if test="${news.audit eq '未审核'}">
									<td><font size="9pt" color="red">${news.audit }</font></td>
								</c:if>
								...
								<td>
									<a class="tablelink" href="updateNews_NewsAction?news.id=${news.id }">过审</a>
