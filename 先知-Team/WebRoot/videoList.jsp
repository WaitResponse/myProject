        ...
        <div class="g-context g-c">
            ...
            <div class="index-lf">
                ...
                <ul>
					<c:choose>
						...
						<c:otherwise>
						<li><a href="Index_preIndexAction">全部</a></li>
					 	<li>
					 	<a <c:if test="${empty currClassify.id}">class="i-active"</c:if>
					 	 href="getVideoList_preIndexAction">视频</a>
					 	</li>
							<c:forEach items="${classifyList }" var="classify">
							<li>
							<a <c:if test="${currClassify.id==classify.id }">class="i-active"</c:if>
							href="getNewsListByType_preIndexAction?classify.id=${classify.id }">
							${classify.name }</a></li>								
							</c:forEach>
						</c:otherwise>
					</c:choose>                                   
            ...
            <div class="index-md">
                ...
                <div class="list-news-container">               
                    <ul class="news-list">
                    	<c:choose>
                    		<c:when test="${empty videoList1}">
                    			<li>暂无视频</li>
                    		</c:when>
                    		<c:otherwise>
                    			<c:forEach items="${videoList1}" var="video">
                    				 <li style='padding-top: 0'>
                          		  <div class="item-inner-one item-inner-same g-c">
                          	     <div>
                                    <div class="item-inner-img sc-tr">
                                        <a href="getVideoDetail_preIndexAction?video.id=${video.id }" target="_blank">
                                            <img src="${video.picture}"/>                                   
                                        </a>
                                    </div>
                                    <div class="item-inner-title">
                                        <span class="item-inner-center">
                                            <a class="it-news-tl" href="getVideoDetail_preIndexAction?video.id=${video.id }" target="_blank">${video.title }</a>
                                            <p class="it-news-footer">
                                                <a class="item-news-type item-news-same" href="getVideoList_preIndexAction">视频</a>                                              
                                                <a href="loadCollectNews_UserCenterAction?user.id=${video.author.id }" class="item-news-same item-news-name">${video.author.nickName }</a>
                                                <span class="item-news-same" href="#"><font color="blue">${video.discussCount }</font>&nbsp;评论</span>
                                                &nbsp;
                                                <time class="timeago" datetime="${video.date }"></time>                 
                                            </p>
                                        </span>
                                    </div>
                                </div>
